import {
    ...
    FlatList,
    SafeAreaView,
    KeyboardAvoidingViewBase, Platform
} from "react-native";
...
export default function ColorPaletteModal({navigation}) {
    ...
    const handleSubmit = useCallback(() => {
        if (!name) {
            ...
            navigation.navigate('Home', {newColorPalette: {paletteName: name, colors: selectedColors}});
        }
    ...
    const handleValueChange = useCallback((selected, color) => {
        if (selected) {
    ...
    return (
        <SafeAreaView style={{flex: 1}}>
            <View>
                <Text style={styles.text_1}>Name of the Palette</Text>
                <TextInput style={styles.input} placeholder="Palette Name" value={name} onChangeText={setName}/>
                <View style={{paddingTop: 10}}>
                    <FlatList data={Colours} keyExtractor={item => item.colorName} renderItem={({item}) => (
                        <View style={styles.view}>
                            <Text style={styles.text}>{item.colorName}</Text>
                            <Switch value={!!selectedColors.find(
